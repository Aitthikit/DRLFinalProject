...
import double_inv_pen.tasks.double_inverted_pendulum.mdp as mdp
...
CART_DOUBLE_PENDULUM_CFG = ArticulationCfg(
    ...
    init_state=ArticulationCfg.InitialStateCfg(
        pos=(5.0, 0.0, 2.0), joint_pos={"SlidertoCart": 0.0, "CarttoPole": 0.0, "Pole1toPole2": 0.0}
    ),
    actuators={
        ...
        "pendulum_actuator": ImplicitActuatorCfg(
            joint_names_expr=["Pole1toPole2"], effort_limit=400.0, velocity_limit=100.0, stiffness=0.0, damping=0.0
        ),
    },
)
...
class ActionsCfg:
    ...
    # joint_effort = mdp.JointVelocityActionCfg(asset_name="robot", joint_names=["SlidertoCart"], scale=1.0)
    joint_effort = mdp.JointEffortActionCfg(asset_name="robot", joint_names=["SlidertoCart"], scale=1.0)
...
#     joint_pos2 = RewTerm(func=mdp.joint_pos_target_l2,weight = -1.0 ,params={"asset_cfg": SceneEntityCfg("robot",joint_names = ["Pole1toPole2"]),"target" : 0.0})
...
#     # (3) Joint_pos
#     cart_pos = RewTerm(func=mdp.joint_pos_target_l2,weight = -50.0 ,params={"asset_cfg": SceneEntityCfg("robot",joint_names = ["SlidertoCart"]),"target" : 0.0})

#     joint_pos = RewTerm(func=mdp.joint_pos_target_l2,weight = -100.0 ,params={"asset_cfg": SceneEntityCfg("robot",joint_names = ["CarttoPole"]),"target" : 0.0})

#     joint_pos2 = RewTerm(func=mdp.joint_pos_target_l2,weight = -100.0 ,params={"asset_cfg": SceneEntityCfg("robot",joint_names = ["Pole1toPole2"]),"target" : 0.0})
#     # (4) Shaping tasks: lower cart velocity
...
@configclass
class Stabilize_DoubleCartpoleEnvCfg(ManagerBasedRLEnvCfg):
