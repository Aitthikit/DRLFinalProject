...
- Throughout the repository, the name `double_inv_pen` only serves as an example and we provide a script to rename all the references to it automatically:

```bash
# Enter the repository
cd IsaacLabExtensionTemplate
# Rename all occurrences of double_inv_pen (in files/directories) to your_fancy_extension_name
...
```bash
python -m pip install -e source/double_inv_pen
```
...
We provide an example UI extension that will load upon enabling your extension defined in `source/double_inv_pen/double_inv_pen/ui_extension_example.py`.
...
```json
{
    "python.analysis.extraPaths": [
        "<path-to-ext-repo>/source/double_inv_pen"
    ]
}
```
