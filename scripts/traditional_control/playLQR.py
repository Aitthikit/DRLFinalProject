...
def main():
    ...
    log_root_path = os.path.join("logs", "mpc", args_cli.task)
    ...
    obs_history = []
    test = "LQR"
    K = np.array([7.07106781186614,    344.958659984289 ,   2062.92226835528   , 32.0934664302521  , 229.609838942768   , 365.161174123341])
    # mpc = MPC() 
    ...
    while simulation_app.is_running():
        ...
        with torch.inference_mode():
            ...
            # obsArray = torch.tensor(obs[0], device='cuda:0')
            # offsets = np.array([0.0, 3.14, -1.57, 0.0, 0.0, 0.0])
            
            action = -np.dot(K,  (obs[0]-x0))
            # action = np.clip(action, -30, 30)
            print(action)

            action_use = torch.tensor(action*-1,device='cuda:0', requires_grad=True).unsqueeze(0).unsqueeze(0)
            # env stepping
            obs_history.append(obs)
            

            obs, _, _, _ = env.step(action_use)
            time.sleep(1)
            timestep2 += 1
            # print(obs)
        if timestep2 >= 3000:
